...
const handleClick = (event)=>{
  ...
  const handleSubmit = (event) => {
    ...
    use.loadQnA().then(model => {
      ...
      const input = {
        queries: [userInput],
        responses: [userInput, ...randomWords]
      };
      const embeddings = model.embed(input);
      const scores = tf.matMul(embeddings['queryEmbedding'],
        embeddings['responseEmbedding'], false, true).dataSync();
  ...
  const dummy = [userInput, ...randomWords ]
  ...
  return (
    <div className="App">
      ...
      <form onSubmit={handleSubmit}>
        ...
        <button>check</button>
        <button onClick={handleClick}>change words</button>
      </form>
      {dummy.map((ele, idx) =>
